...
namespace Cistern.Linq.ChainLinq.Consumables
{
    ...
    {
        Consumable<V> IMergeSelect<T>.MergeSelect<V>(ConsumableCons<T> _, Func<T, V> u2v) =>
            new WhereSelectArray<T, V>(Underlying, Predicate, u2v);

        Consumable<T> IMergeWhere<T>.MergeWhere(ConsumableCons<T> _, Func<T, bool> predicate) =>
    ...
    {
        Consumable<V> IMergeSelect<T>.MergeSelect<V>(ConsumableCons<T> consumable, Func<T, V> u2v) =>
            new WhereSelectEnumerable<TEnumerable, TEnumerator, T, V>(Underlying, Predicate, u2v);

        Consumable<T> IMergeWhere<T>.MergeWhere(ConsumableCons<T> consumable, Func<T, bool> predicate) =>
