...
module Cistern.Linq.FSharp.Tests.LinqInternalConsistency
...
/// to force the use of the GetEnumerable
...
let list x              = x |> Linq.toList
let list_map x          = x |> Linq.toList |> Linq.map id
let list_filter x       = x |> Linq.toList |> Linq.filter (fun _ -> true)
let list_mapfilter x    = x |> Linq.toList |> Linq.map id |> Linq.filter (fun _ -> true) 
let list_filtermap x    = x |> Linq.toList |> Linq.filter (fun _ -> true) |> Linq.map id
let array x             = x |> Linq.toArray
let array_map x         = x |> Linq.toArray |> Linq.map id
let array_filter x      = x |> Linq.toArray |> Linq.filter (fun _ -> true)
let array_mapfilter x   = x |> Linq.toArray |> Linq.map id |> Linq.filter (fun _ -> true) 
let array_filtermap x   = x |> Linq.toArray |> Linq.filter (fun _ -> true) |> Linq.map id

/// helper function that creates labeled FsCheck properties for equality comparisons
let consistency name items =
    match items with
    | first :: second :: rest ->
        ((first = second) |@ (sprintf "%s: %A, %A" name first second), rest)
        ||> List.fold (fun acc current -> acc .&. (first = current) |@ (sprintf "%A" current))
    | _ -> failwith "Expected more items"

let allPairs<'a when 'a : equality> (xs : list<'a>) (xs2 : list<'a>) =
    let _00 = (xs |> list,            xs2) ||> Linq.allPairs |> list            |> toArray
    let _01 = (xs |> list_map,        xs2) ||> Linq.allPairs |> list_map        |> toArray
    let _02 = (xs |> list_filter,     xs2) ||> Linq.allPairs |> list_filter     |> toArray
    let _03 = (xs |> list_mapfilter,  xs2) ||> Linq.allPairs |> list_mapfilter  |> toArray
    let _04 = (xs |> list_filtermap,  xs2) ||> Linq.allPairs |> list_filtermap  |> toArray
    let _05 = (xs |> array,           xs2) ||> Linq.allPairs |> array           |> toArray
    let _06 = (xs |> array_map,       xs2) ||> Linq.allPairs |> array_map       |> toArray
    let _07 = (xs |> array_filter,    xs2) ||> Linq.allPairs |> array_filter    |> toArray
    let _08 = (xs |> array_mapfilter, xs2) ||> Linq.allPairs |> array_mapfilter |> toArray
    let _09 = (xs |> array_filtermap, xs2) ||> Linq.allPairs |> array_filtermap |> toArray
    let _10 = (xs |> list,            xs2) ||> Linq.allPairs |> list            |> Linq.toArray
    let _11 = (xs |> list_map,        xs2) ||> Linq.allPairs |> list_map        |> Linq.toArray
    let _12 = (xs |> list_filter,     xs2) ||> Linq.allPairs |> list_filter     |> Linq.toArray
    let _13 = (xs |> list_mapfilter,  xs2) ||> Linq.allPairs |> list_mapfilter  |> Linq.toArray
    let _14 = (xs |> list_filtermap,  xs2) ||> Linq.allPairs |> list_filtermap  |> Linq.toArray
    let _15 = (xs |> array,           xs2) ||> Linq.allPairs |> array           |> Linq.toArray
    let _16 = (xs |> array_map,       xs2) ||> Linq.allPairs |> array_map       |> Linq.toArray
    let _17 = (xs |> array_filter,    xs2) ||> Linq.allPairs |> array_filter    |> Linq.toArray
    let _18 = (xs |> array_mapfilter, xs2) ||> Linq.allPairs |> array_mapfilter |> Linq.toArray
    let _19 = (xs |> array_filtermap, xs2) ||> Linq.allPairs |> array_filtermap |> Linq.toArray
    consistency "allPairs" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09;_10;_11;_12;_13;_14;_15;_16;_17;_18;_19]
...
let append<'a when 'a : equality> (xs : list<'a>) (xs2 : list<'a>) =
    let _00 = (xs |> list,            xs2) ||> Linq.append |> list            |> toArray
    let _01 = (xs |> list_map,        xs2) ||> Linq.append |> list_map        |> toArray
    let _02 = (xs |> list_filter,     xs2) ||> Linq.append |> list_filter     |> toArray
    let _03 = (xs |> list_mapfilter,  xs2) ||> Linq.append |> list_mapfilter  |> toArray
    let _04 = (xs |> list_filtermap,  xs2) ||> Linq.append |> list_filtermap  |> toArray
    let _05 = (xs |> array,           xs2) ||> Linq.append |> array           |> toArray
    let _06 = (xs |> array_map,       xs2) ||> Linq.append |> array_map       |> toArray
    let _07 = (xs |> array_filter,    xs2) ||> Linq.append |> array_filter    |> toArray
    let _08 = (xs |> array_mapfilter, xs2) ||> Linq.append |> array_mapfilter |> toArray
    let _09 = (xs |> array_filtermap, xs2) ||> Linq.append |> array_filtermap |> toArray
    let _10 = (xs |> list,            xs2) ||> Linq.append |> list            |> Linq.toArray
    let _11 = (xs |> list_map,        xs2) ||> Linq.append |> list_map        |> Linq.toArray
    let _12 = (xs |> list_filter,     xs2) ||> Linq.append |> list_filter     |> Linq.toArray
    let _13 = (xs |> list_mapfilter,  xs2) ||> Linq.append |> list_mapfilter  |> Linq.toArray
    let _14 = (xs |> list_filtermap,  xs2) ||> Linq.append |> list_filtermap  |> Linq.toArray
    let _15 = (xs |> array,           xs2) ||> Linq.append |> array           |> Linq.toArray
    let _16 = (xs |> array_map,       xs2) ||> Linq.append |> array_map       |> Linq.toArray
    let _17 = (xs |> array_filter,    xs2) ||> Linq.append |> array_filter    |> Linq.toArray
    let _18 = (xs |> array_mapfilter, xs2) ||> Linq.append |> array_mapfilter |> Linq.toArray
    let _19 = (xs |> array_filtermap, xs2) ||> Linq.append |> array_filtermap |> Linq.toArray
    consistency "append" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09;_10;_11;_12;_13;_14;_15;_16;_17;_18;_19]
...
let averageFloat (xs : NormalFloat []) =
    let xs = xs |> Array.map float |> Array.toList
    let _00 = runAndCheckErrorType (fun () -> xs |> list            |> Linq.average)
    let _01 = runAndCheckErrorType (fun () -> xs |> list_map        |> Linq.average)
    let _02 = runAndCheckErrorType (fun () -> xs |> list_filter     |> Linq.average)
    let _03 = runAndCheckErrorType (fun () -> xs |> list_mapfilter  |> Linq.average)
    let _04 = runAndCheckErrorType (fun () -> xs |> list_filtermap  |> Linq.average)
    let _05 = runAndCheckErrorType (fun () -> xs |> array           |> Linq.average)
    let _06 = runAndCheckErrorType (fun () -> xs |> array_map       |> Linq.average)
    let _07 = runAndCheckErrorType (fun () -> xs |> array_filter    |> Linq.average)
    let _08 = runAndCheckErrorType (fun () -> xs |> array_mapfilter |> Linq.average)
    let _09 = runAndCheckErrorType (fun () -> xs |> array_filtermap |> Linq.average)
    consistency "average" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09]
...
let averageBy (xs : float []) f =
    ...
    let _00 = runAndCheckErrorType (fun () -> xs |> list            |> Linq.averageBy f)
    let _01 = runAndCheckErrorType (fun () -> xs |> list_map        |> Linq.averageBy f)
    let _02 = runAndCheckErrorType (fun () -> xs |> list_filter     |> Linq.averageBy f)
    let _03 = runAndCheckErrorType (fun () -> xs |> list_mapfilter  |> Linq.averageBy f)
    let _04 = runAndCheckErrorType (fun () -> xs |> list_filtermap  |> Linq.averageBy f)
    let _05 = runAndCheckErrorType (fun () -> xs |> array           |> Linq.averageBy f)
    let _06 = runAndCheckErrorType (fun () -> xs |> array_map       |> Linq.averageBy f)
    let _07 = runAndCheckErrorType (fun () -> xs |> array_filter    |> Linq.averageBy f)
    let _08 = runAndCheckErrorType (fun () -> xs |> array_mapfilter |> Linq.averageBy f)
    let _09 = runAndCheckErrorType (fun () -> xs |> array_filtermap |> Linq.averageBy f)
    consistency "averageBy" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09]
...
let contains<'a when 'a : equality> (xs : 'a []) x  =
    let _00 = xs |> list            |> Linq.contains x
    let _01 = xs |> list_map        |> Linq.contains x
    let _02 = xs |> list_filter     |> Linq.contains x
    let _03 = xs |> list_mapfilter  |> Linq.contains x
    let _04 = xs |> list_filtermap  |> Linq.contains x
    let _05 = xs |> array           |> Linq.contains x
    let _06 = xs |> array_map       |> Linq.contains x
    let _07 = xs |> array_filter    |> Linq.contains x
    let _08 = xs |> array_mapfilter |> Linq.contains x
    let _09 = xs |> array_filtermap |> Linq.contains x
    consistency "contains" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09]
...
let choose<'a when 'a : equality> (xs : 'a []) f  =
    let _00 = xs |> list            |> Linq.choose f |> list            |> toArray
    let _01 = xs |> list_map        |> Linq.choose f |> list_map        |> toArray
    let _02 = xs |> list_filter     |> Linq.choose f |> list_filter     |> toArray
    let _03 = xs |> list_mapfilter  |> Linq.choose f |> list_mapfilter  |> toArray
    let _04 = xs |> list_filtermap  |> Linq.choose f |> list_filtermap  |> toArray
    let _05 = xs |> array           |> Linq.choose f |> array           |> toArray
    let _06 = xs |> array_map       |> Linq.choose f |> array_map       |> toArray
    let _07 = xs |> array_filter    |> Linq.choose f |> array_filter    |> toArray
    let _08 = xs |> array_mapfilter |> Linq.choose f |> array_mapfilter |> toArray
    let _09 = xs |> array_filtermap |> Linq.choose f |> array_filtermap |> toArray
    let _10 = xs |> list            |> Linq.choose f |> list            |> Linq.toArray
    let _11 = xs |> list_map        |> Linq.choose f |> list_map        |> Linq.toArray
    let _12 = xs |> list_filter     |> Linq.choose f |> list_filter     |> Linq.toArray
    let _13 = xs |> list_mapfilter  |> Linq.choose f |> list_mapfilter  |> Linq.toArray
    let _14 = xs |> list_filtermap  |> Linq.choose f |> list_filtermap  |> Linq.toArray
    let _15 = xs |> array           |> Linq.choose f |> array           |> Linq.toArray
    let _16 = xs |> array_map       |> Linq.choose f |> array_map       |> Linq.toArray
    let _17 = xs |> array_filter    |> Linq.choose f |> array_filter    |> Linq.toArray
    let _18 = xs |> array_mapfilter |> Linq.choose f |> array_mapfilter |> Linq.toArray
    let _19 = xs |> array_filtermap |> Linq.choose f |> array_filtermap |> Linq.toArray
    consistency "choose" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09;_10;_11;_12;_13;_14;_15;_16;_17;_18;_19]
...
let chunkBySize<'a when 'a : equality> (xs : 'a []) size =
    ...
    if size <= 0 then 
        Prop.throws<ArgumentException,_> (lazy Linq.chunkBySize size xs) .&.
        ...
        let _00 = xs |> list            |> Linq.chunkBySize size |> list            |> toArray
        let _01 = xs |> list_map        |> Linq.chunkBySize size |> list_map        |> toArray
        let _02 = xs |> list_filter     |> Linq.chunkBySize size |> list_filter     |> toArray
        let _03 = xs |> list_mapfilter  |> Linq.chunkBySize size |> list_mapfilter  |> toArray
        let _04 = xs |> list_filtermap  |> Linq.chunkBySize size |> list_filtermap  |> toArray
        let _05 = xs |> array           |> Linq.chunkBySize size |> array           |> toArray
        let _06 = xs |> array_map       |> Linq.chunkBySize size |> array_map       |> toArray
        let _07 = xs |> array_filter    |> Linq.chunkBySize size |> array_filter    |> toArray
        let _08 = xs |> array_mapfilter |> Linq.chunkBySize size |> array_mapfilter |> toArray
        let _09 = xs |> array_filtermap |> Linq.chunkBySize size |> array_filtermap |> toArray
        let _10 = xs |> list            |> Linq.chunkBySize size |> list            |> Linq.toArray
        let _11 = xs |> list_map        |> Linq.chunkBySize size |> list_map        |> Linq.toArray
        let _12 = xs |> list_filter     |> Linq.chunkBySize size |> list_filter     |> Linq.toArray
        let _13 = xs |> list_mapfilter  |> Linq.chunkBySize size |> list_mapfilter  |> Linq.toArray
        let _14 = xs |> list_filtermap  |> Linq.chunkBySize size |> list_filtermap  |> Linq.toArray
        let _15 = xs |> array           |> Linq.chunkBySize size |> array           |> Linq.toArray
        let _16 = xs |> array_map       |> Linq.chunkBySize size |> array_map       |> Linq.toArray
        let _17 = xs |> array_filter    |> Linq.chunkBySize size |> array_filter    |> Linq.toArray
        let _18 = xs |> array_mapfilter |> Linq.chunkBySize size |> array_mapfilter |> Linq.toArray
        let _19 = xs |> array_filtermap |> Linq.chunkBySize size |> array_filtermap |> Linq.toArray
        consistency "chunkBySize" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09;_10;_11;_12;_13;_14;_15;_16;_17;_18;_19]
...
let collect<'a> (xs : 'a []) (f:'a->array<obj>)  =
    let _00 = xs |> list            |> Linq.collect f |> list            |> toArray
    let _01 = xs |> list_map        |> Linq.collect f |> list_map        |> toArray
    let _02 = xs |> list_filter     |> Linq.collect f |> list_filter     |> toArray
    let _03 = xs |> list_mapfilter  |> Linq.collect f |> list_mapfilter  |> toArray
    let _04 = xs |> list_filtermap  |> Linq.collect f |> list_filtermap  |> toArray
    let _05 = xs |> array           |> Linq.collect f |> array           |> toArray
    let _06 = xs |> array_map       |> Linq.collect f |> array_map       |> toArray
    let _07 = xs |> array_filter    |> Linq.collect f |> array_filter    |> toArray
    let _08 = xs |> array_mapfilter |> Linq.collect f |> array_mapfilter |> toArray
    let _09 = xs |> array_filtermap |> Linq.collect f |> array_filtermap |> toArray
    let _10 = xs |> list            |> Linq.collect f |> list            |> Linq.toArray
    let _11 = xs |> list_map        |> Linq.collect f |> list_map        |> Linq.toArray
    let _12 = xs |> list_filter     |> Linq.collect f |> list_filter     |> Linq.toArray
    let _13 = xs |> list_mapfilter  |> Linq.collect f |> list_mapfilter  |> Linq.toArray
    let _14 = xs |> list_filtermap  |> Linq.collect f |> list_filtermap  |> Linq.toArray
    let _15 = xs |> array           |> Linq.collect f |> array           |> Linq.toArray
    let _16 = xs |> array_map       |> Linq.collect f |> array_map       |> Linq.toArray
    let _17 = xs |> array_filter    |> Linq.collect f |> array_filter    |> Linq.toArray
    let _18 = xs |> array_mapfilter |> Linq.collect f |> array_mapfilter |> Linq.toArray
    let _19 = xs |> array_filtermap |> Linq.collect f |> array_filtermap |> Linq.toArray
    consistency "collect" [_00;_01;_02;_03;_04;_05;_06;_07;_08;_09;_10;_11;_12;_13;_14;_15;_16;_17;_18;_19]
...
//    let s = (xs, xs2) ||> Linq.compareWith f
...
//    let s = xs |> Linq.countBy f |> toArray
...
//    let s = xs |> Linq.distinct |> toArray
...
//    let s = xs |> Linq.distinctBy f |> toArray
...
//    let s = runAndCheckErrorType (fun () -> xs |> Linq.exactlyOne)
...
//    let s = runAndCheckErrorType (fun () -> xs |> Linq.tryExactlyOne)
...
//    let s = xs |> Linq.except itemsToExclude |> toArray
...
//    let s = runAndCheckErrorType (fun () -> Linq.exists2 f xs xs2)
...
//    let s = run (fun () -> xs |> Linq.find predicate)
...
//    let s = run (fun () -> xs |> Linq.findBack predicate)
...
//    let s = run (fun () -> xs |> Linq.findIndex predicate)
...
//    let s = run (fun () -> xs |> Linq.findIndexBack predicate)
...
//    let s = run (fun () -> xs |> Linq.fold f start)
...
//    let s = run (fun () -> Linq.fold2 f start xs xs2)
...
//    let s = run (fun () -> Linq.foldBack f xs start)
...
//    let xs = xs' |> Array.map fst
//    let xs2 = xs' |> Array.map snd
//    let s = run (fun () -> Linq.foldBack2 f xs xs2 start)
...
//    let xs = Array.map fst xs'
//    let xs2 = Array.map snd xs'
//    let s = runAndCheckErrorType (fun () -> Linq.forall2 f xs xs2)
...
//    let s = run (fun () -> xs |> Linq.groupBy f |> toArray |> Array.map (fun (x,xs) -> x,xs |> toArray))
//    let l = run (fun () -> xs |> List.ofArray |> List.groupBy f |> toArray |> Array.map (fun (x,xs) -> x,xs |> toArray))
//    let a = run (fun () -> xs |> Array.groupBy f |> Array.map (fun (x,xs) -> x,xs |> toArray))
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.head)
...
//    let s = runAndCheckErrorType (fun () -> Linq.init count f |> toArray)
//    let l = runAndCheckErrorType (fun () -> List.init count f |> toArray)
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.item index)
...
//    let s = xs |> Linq.iter f
//    let l = xs |> List.ofArray |> List.iter f
//    let a =  xs |> Array.iter f

//    let xs = Linq.toList xs
//    list |> Linq.toList = (xs @ xs @ xs)
...
//    let s = xs |> Linq.iteri f
//    let l = xs |> List.ofArray |> List.iteri f
//    let a =  xs |> Array.iteri f

//    let xs = Linq.toList xs
//    list |> Linq.toList = (xs @ xs @ xs) &&
//      indices |> Linq.toList = ([0..xs.Length-1] @ [0..xs.Length-1] @ [0..xs.Length-1])
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.last)
...
//    let s,sr = xs |> Linq.mapFold f start
...
//    let s,sr = Linq.mapFoldBack f xs start
...
//    toArray s = a && List.toArray l = a &&
//      list |> Linq.toList = (xs @ xs @ xs) &&
//      (Linq.toList indices = [0..xs.Length-1] @ [0..xs.Length-1] @ [0..xs.Length-1])
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.max)
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.maxBy f)
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.min)
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.minBy f)
...
//    let s = run (fun () -> xs |> Linq.pairwise |> toArray)
...
//    let s = run (fun () -> xs |> Linq.permute permutation |> toArray)
...
//    let s = runAndCheckErrorType (fun () -> xs |> Linq.reduce f)
...
//    let s = runAndCheckErrorType (fun () -> xs |> Linq.reduceBack f)
...
//    let s = runAndCheckIfAnyError (fun () -> Linq.replicate count x |> toArray)
...
//    let s = run (fun () -> xs |> Linq.scan f start |> toArray)
//    let l = run (fun () -> xs |> List.ofArray |> List.scan f start |> toArray)
...
//    let s = run (fun () -> Linq.scanBack f xs start |> toArray)
//    let l = run (fun () -> List.scanBack f (xs |> List.ofArray) start |> toArray)
...
//    let s = Linq.singleton x |> toArray
...
//    let s = runAndCheckIfAnyError (fun () -> Linq.skip count xs |> toArray)
//    let l = runAndCheckIfAnyError (fun () -> List.skip count (Linq.toList xs) |> List.toArray)
...
//    let s = runAndCheckIfAnyError (fun () -> Linq.skipWhile f xs |> toArray)
//    let l = runAndCheckIfAnyError (fun () -> List.skipWhile f (Linq.toList xs) |> List.toArray)
...
//    let s = xs |> Linq.sortBy f
//    let l = xs |> List.ofArray |> List.sortBy f
//    let a = xs |> Array.sortBy f

//    isSorted (Linq.map f s) && isSorted (Linq.map f l) && isSorted (Linq.map f a) &&
...
//    let s = xs |> Linq.sortWith f
//    let l = xs |> List.ofArray |> List.sortWith f
//    let a = xs |> Array.sortWith f
//    let isSorted sorted = sorted |> Linq.pairwise |> Linq.forall (fun (a,b) -> f a b <= 0 || a = b)
...
//    let s = xs |> Linq.sortDescending |> toArray
...
//    let s = xs |> Linq.sortByDescending f
//    let l = xs |> List.ofArray |> List.sortByDescending f
//    let a = xs |> Array.sortByDescending f

//    isSorted (Linq.map f s |> Linq.rev) && isSorted (Linq.map f l |> Linq.rev) && isSorted (Linq.map f a |> Linq.rev) &&
//      haveSameElements s xs && haveSameElements l xs && haveSameElements a xs
...
//        Prop.throws<ArgumentException,_> (lazy Linq.splitInto count xs) 
//    else
//        let s = run (fun () -> xs |> Linq.splitInto count |> Linq.map toArray |> toArray)
//        let l = run (fun () -> ls |> List.splitInto count |> Linq.map toArray |> toArray)
//        let a = run (fun () -> xs |> Array.splitInto count |> Linq.map toArray |> toArray)
...
//    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.tail |> toArray)
//    let l = runAndCheckIfAnyError (fun () -> xs |> List.ofArray |> List.tail |> toArray)
...
//    let s = runAndCheckIfAnyError (fun () -> Linq.take count xs |> toArray)
//    let l = runAndCheckIfAnyError (fun () -> List.take count (Linq.toList xs) |> List.toArray)
...
//    let s = runAndCheckIfAnyError (fun () -> Linq.takeWhile f xs |> toArray)
//    let l = runAndCheckIfAnyError (fun () -> List.takeWhile f (Linq.toList xs) |> List.toArray)
...
//    let s = runAndCheckIfAnyError (fun () -> Linq.truncate count xs |> toArray)
//    let l = runAndCheckIfAnyError (fun () -> List.truncate count (Linq.toList xs) |> List.toArray)
...
//    let s = xs |> Linq.tryFind predicate
...
//    let s = xs |> Linq.tryFindBack predicate
...
//    let s = xs |> Linq.tryFindIndex predicate
...
//    let s = xs |> Linq.tryFindIndexBack predicate
...
//            c := !c + 1
//            f x
//    let s : 'b [] = Linq.unfold (f()) start |> toArray
...
//    let s = xs |> Linq.where predicate |> toArray
...
//        Prop.throws<ArgumentException,_> (lazy   Linq.windowed windowSize xs) .&.
...
//    else
//        let s = run (fun () -> xs |> Linq.windowed windowSize |> toArray |> Array.map toArray)
//        let l = run (fun () -> ls |> List.windowed windowSize |> List.toArray |> Array.map toArray)
...
//    let xs = Array.map fst xs'
//    let xs2 = Array.map snd xs'
//    let s = runAndCheckErrorType (fun () -> Linq.zip xs xs2 |> toArray)
...
//    let s = runAndCheckErrorType (fun () -> Linq.zip3 xs xs2 xs3 |> toArray)
