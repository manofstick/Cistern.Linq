...
let append<'a when 'a : equality> (xs : list<'a>) (xs2 : list<'a>) =
    let s = xs |> Linq.append xs2 |> Linq.toArray
...
let averageFloat (xs : NormalFloat []) =
    ...
    let s = runAndCheckErrorType (fun () -> xs |> Linq.average)
...
let averageBy (xs : float []) f =
    ...
    let s = runAndCheckErrorType (fun () -> xs |> Linq.averageBy f)
...
let choose<'a when 'a : equality> (xs : 'a []) f  =
    let s = xs |> Linq.choose f |> Linq.toArray
...
let chunkBySize<'a when 'a : equality> (xs : 'a []) size =
    ...
    if size <= 0 then 
        ...
        let s = xs |> Linq.chunkBySize size |> Linq.map Linq.toArray |> Linq.toArray
        let l = ls |> List.chunkBySize size |> Linq.map Linq.toArray |> Linq.toArray
        let a = xs |> Array.chunkBySize size |> Linq.map Linq.toArray |> Linq.toArray
...
let collect<'a> (xs : 'a []) f  =
    let s = xs |> Linq.collect f |> Linq.toArray
...
let concat<'a when 'a : equality> (xs : 'a [][]) =
    let s = xs |> Linq.concat |> Linq.toArray
...
let countBy<'a> (xs : 'a []) f =
    let s = xs |> Linq.countBy f |> Linq.toArray
...
let distinct<'a when 'a : comparison> (xs : 'a []) =
    let s = xs |> Linq.distinct |> Linq.toArray
...
let distinctBy<'a when 'a : equality> (xs : 'a []) f =
    let s = xs |> Linq.distinctBy f |> Linq.toArray
...
let except<'a when 'a : equality> (xs : 'a []) (itemsToExclude: 'a []) =
    let s = xs |> Linq.except itemsToExclude |> Linq.toArray
...
let filter<'a when 'a : equality> (xs : 'a []) predicate =
    ...
    Linq.toArray s = a && List.toArray l = a
...
let groupBy<'a when 'a : equality> (xs : 'a []) f =
    let s = run (fun () -> xs |> Linq.groupBy f |> Linq.toArray |> Array.map (fun (x,xs) -> x,xs |> Linq.toArray))
    let l = run (fun () -> xs |> List.ofArray |> List.groupBy f |> Linq.toArray |> Array.map (fun (x,xs) -> x,xs |> Linq.toArray))
    let a = run (fun () -> xs |> Array.groupBy f |> Array.map (fun (x,xs) -> x,xs |> Linq.toArray))
...
let indexed<'a when 'a : equality> (xs : 'a []) =
    let s = xs |> Linq.indexed |> Linq.toArray
...
let init<'a when 'a : equality> count f =
    let s = runAndCheckErrorType (fun () -> Linq.init count f |> Linq.toArray)
    let l = runAndCheckErrorType (fun () -> List.init count f |> Linq.toArray)
...
let map<'a when 'a : equality> (xs : 'a []) f =
    let s = xs |> Linq.map f |> Linq.toArray
...
let pairwise<'a when 'a : comparison> (xs : 'a []) =
    let s = run (fun () -> xs |> Linq.pairwise |> Linq.toArray)
...
let permute<'a when 'a : comparison> (xs' : list<int*'a>) =
    ...
    let s = run (fun () -> xs |> Linq.permute permutation |> Linq.toArray)
...
let replicate<'a when 'a : equality> x count =
    let s = runAndCheckIfAnyError (fun () -> Linq.replicate count x |> Linq.toArray)
...
let rev<'a when 'a : equality> (xs : 'a []) =
    let s = Linq.rev xs |> Linq.toArray
...
let scan<'a,'b when 'b : equality> (xs : 'a []) f (start:'b) =
    let s = run (fun () -> xs |> Linq.scan f start |> Linq.toArray)
    let l = run (fun () -> xs |> List.ofArray |> List.scan f start |> Linq.toArray)
...
let scanBack<'a,'b when 'b : equality> (xs : 'a []) f (start:'b) =
    let s = run (fun () -> Linq.scanBack f xs start |> Linq.toArray)
    let l = run (fun () -> List.scanBack f (xs |> List.ofArray) start |> Linq.toArray)
...
let singleton<'a when 'a : equality> (x : 'a) =
    let s = Linq.singleton x |> Linq.toArray
...
let skip<'a when 'a : equality> (xs : 'a []) count =
    let s = runAndCheckIfAnyError (fun () -> Linq.skip count xs |> Linq.toArray)
...
let skipWhile<'a when 'a : equality> (xs : 'a []) f =
    let s = runAndCheckIfAnyError (fun () -> Linq.skipWhile f xs |> Linq.toArray)
...
let sort<'a when 'a : comparison> (xs : 'a []) =
    let s = xs |> Linq.sort |> Linq.toArray
...
let sortDescending<'a when 'a : comparison> (xs : 'a []) =
    let s = xs |> Linq.sortDescending |> Linq.toArray
...
let splitInto<'a when 'a : equality> (xs : 'a []) count =
    ...
    if count < 1 then 
        ...
        let s = run (fun () -> xs |> Linq.splitInto count |> Linq.map Linq.toArray |> Linq.toArray)
        let l = run (fun () -> ls |> List.splitInto count |> Linq.map Linq.toArray |> Linq.toArray)
        let a = run (fun () -> xs |> Array.splitInto count |> Linq.map Linq.toArray |> Linq.toArray)
...
let tail<'a when 'a : equality> (xs : 'a []) =
    let s = runAndCheckIfAnyError (fun () -> xs |> Linq.tail |> Linq.toArray)
    let l = runAndCheckIfAnyError (fun () -> xs |> List.ofArray |> List.tail |> Linq.toArray)
...
let take<'a when 'a : equality> (xs : 'a []) count =
    let s = runAndCheckIfAnyError (fun () -> Linq.take count xs |> Linq.toArray)
...
let takeWhile<'a when 'a : equality> (xs : 'a []) f =
    let s = runAndCheckIfAnyError (fun () -> Linq.takeWhile f xs |> Linq.toArray)
...
let truncate<'a when 'a : equality> (xs : 'a []) count =
    let s = runAndCheckIfAnyError (fun () -> Linq.truncate count xs |> Linq.toArray)
...
let unfold<'a,'b when 'b : equality> f (start:'a) =
    let f() =
        ...
        fun x -> 
            ...
            c := !c + 1
            f x
    let s : 'b [] = Linq.unfold (f()) start |> Linq.toArray
...
let where<'a when 'a : equality> (xs : 'a []) predicate =
    let s = xs |> Linq.where predicate |> Linq.toArray
...
let windowed<'a when 'a : equality> (xs : 'a []) windowSize =
    ...
    if windowSize < 1 then 
        ...
        let s = run (fun () -> xs |> Linq.windowed windowSize |> Linq.toArray |> Array.map Linq.toArray)
        let l = run (fun () -> ls |> List.windowed windowSize |> List.toArray |> Array.map Linq.toArray)
...
let zip<'a when 'a : equality> (xs':('a*'a) []) =    
    let xs = Array.map fst xs'
    let xs2 = Array.map snd xs'
    let s = runAndCheckErrorType (fun () -> Linq.zip xs xs2 |> Linq.toArray)
...
let zip3<'a when 'a : equality> (xs':('a*'a*'a) []) =    
    ...
    let s = runAndCheckErrorType (fun () -> Linq.zip3 xs xs2 xs3 |> Linq.toArray)
